<template>
  <div class="bg-gray-200">
  <div class="ml-5 grid grid-cols-2 bg-gray-300">
    <h1 class="flex justify-start text-2xl mb-5 ml-20 font-bold">I'm Emerson David Escobar</h1>

    <p class="text-3xl font-bold"></p>


    <div class="max-w-sm rounded overflow-hidden shadow-lg ml-20">
      <img class="w-80 object-cover rounded-xl" src="./assets/yo.jpg" alt="70">
      <div class="px-6 py-4">
        <p class="text-3xl font-bold">About Me</p>
        <p class="text-gray-700 text-base">
        <p>I'm a very proactive person I always loved programming specially back-end area but I adapt to every situation
          also I like to learn a lot of new technologies to be always updated in what's new in the world of technology
          
        </p>
        
        </p>

      </div>
      <div class="px-6 pt-4 pb-2">
        <span
          class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">#Alto</span>
        <span
          class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">#Bronceado</span>
        <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">#Y
          Guapo</span>
      </div>
    </div>
    <div class="">
      <p class="text-3xl font-bold">Habilidades Tecnicas</p>
      <hr class="bg-blue-600 h-1 w-96">
      <div class="mb-3 mt-5 flex items-center ">
        <img src="./assets/sql-cover-blog.png" width="40" alt="40">
        &nbsp; Manejo de herramienta de Base SqlServer
      </div>
      <div class="mb-3 mt-5 flex items-center ">
        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="20.44921875" viewBox="0 0 512 349">
          <path fill="#00758F"
            d="m152.31 230.297l15.56 50.487c3.496 11.463 4.954 19.465 4.37 24.026c8.51-22.792 14.456-47.63 17.839-74.513h18.71c-8.045 43.766-18.656 75.57-31.827 95.41c-10.262 15.289-21.504 22.933-33.746 22.933c-3.264 0-7.288-.986-12.063-2.944v-10.55c2.333.342 5.07.525 8.218.525c5.711 0 10.314-1.583 13.816-4.742c4.193-3.849 6.292-8.175 6.292-12.97c0-3.274-1.637-9.993-4.896-20.157l-21.68-67.505h19.406ZM33.223 199.266l28.5 86.956h.176l28.675-86.956h23.428c5.13 43.124 8.16 82.581 9.09 118.346H103.34c-.695-33.433-2.62-64.871-5.768-94.32H97.4l-30.078 94.32H52.28l-29.896-94.32h-.176c-2.218 28.282-3.614 59.72-4.196 94.32H0c1.164-42.08 4.077-81.525 8.739-118.346h24.485Z" />
          <path fill="#F29111"
            d="M352.498 197.51c30.657 0 45.986 19.586 45.986 58.739c0 21.276-4.61 37.347-13.821 48.204c-1.66 1.984-3.495 3.698-5.427 5.286l21.695 10.727l-.021-.001l-7.703 13.302l-28.253-16.485c-4.683 1.387-9.836 2.08-15.451 2.08c-15.053 0-26.297-4.387-33.731-13.15c-8.16-9.694-12.238-24.955-12.238-45.757c0-21.156 4.602-37.166 13.816-48.037c8.392-9.944 20.11-14.909 35.148-14.909Zm-93.88.172c10.957 0 20.92 2.932 29.894 8.775l-4.558 10.157c-7.679-3.264-15.25-4.903-22.716-4.903c-6.058 0-10.726 1.458-13.98 4.392c-3.272 2.908-5.296 6.65-5.296 11.212c0 7.01 4.994 13.089 14.215 18.225a816.32 816.32 0 0 1 9.031 5.011l.688.387l.345.194l.689.387l.344.194l.688.388c6.98 3.935 13.548 7.691 13.548 7.691c9.22 6.545 13.816 13.523 13.816 25.016c0 10.037-3.678 18.276-11.01 24.723c-7.337 6.418-17.194 9.636-29.538 9.636c-11.545 0-22.734-3.704-33.572-11.05l5.07-10.166c9.327 4.675 17.767 7.01 25.346 7.01c7.108 0 12.672-1.587 16.697-4.721c4.017-3.157 6.424-7.56 6.424-13.143c0-7.027-4.888-13.034-13.855-18.073a897.982 897.982 0 0 1-8.395-4.697l-.687-.389c-1.262-.713-2.533-1.435-3.778-2.142l-.675-.384c-6.055-3.444-11.29-6.453-11.29-6.453c-8.964-6.557-13.459-13.592-13.459-25.184c0-9.587 3.352-17.336 10.046-23.231c6.71-5.908 15.367-8.862 25.968-8.862Zm175.895 1.584v103.788h37.238v14.558h-56.124V199.266h18.886Zm57.93 103.833v2.46h-4.094v12.04h-3.13v-12.04h-4.253v-2.46h11.478Zm7.56 0l3.931 9.884l3.611-9.884h4.437v14.5h-2.95v-11.035l-4.11 11.035h-2.127l-4.117-11.035h-.158v11.035h-2.791v-14.5h4.275ZM350.57 212.064c-18.066 0-27.104 14.91-27.104 44.71c0 17.07 2.395 29.448 7.176 37.163c4.428 7.14 11.363 10.703 20.806 10.703c18.066 0 27.103-15.026 27.103-45.064c0-16.831-2.395-29.103-7.17-36.822c-4.433-7.124-11.365-10.69-20.81-10.69Z" />
          <path fill="#00758F"
            d="M303.218 7.333c5.993-14.726 26.948-3.574 35.08 1.57c1.993 1.287 4.279 4.006 6.564 5.011c3.565.14 7.127.419 10.698.568c6.698 1.574 12.972 2.86 18.25 5.866c24.528 14.445 40.495 29.165 55.19 53.479c3.14 5.15 4.709 10.723 7.274 16.296c3.56 8.307 7.56 17.027 11.692 24.882c1.85 3.724 3.281 7.865 5.85 11.01c1.003 1.438 3.852 1.862 5.555 2.721c4.708 2.437 10.412 4.287 14.84 7.147c8.269 5.156 16.264 11.3 23.532 17.59c2.709 2.428 4.555 5.865 7.136 8.433v1.296c-2.291.703-4.574 1.423-6.859 2c-4.991 1.282-9.412.992-14.254 2.275c-2.992.868-6.707 2.013-9.845 2.304l.29.292c1.846 5.275 11.834 9.565 16.402 12.72c5.548 4.004 10.689 8.86 14.827 14.437c1.429 1.423 2.858 2.718 4.28 4.137c.994 1.438 1.274 3.298 2.28 4.58v.434c-1.114-.393-1.915-1.143-2.674-1.927l-.453-.473c-.453-.47-.91-.932-1.431-1.313c-3.148-2.15-6.274-4.722-9.422-6.721c-5.412-3.434-11.689-5.427-17.246-8.874c-3.142-2.001-6.137-4.28-9.132-6.57c-2.715-2.007-5.705-5.861-7.411-8.721c-1.005-1.58-1.143-3.437-2.291-4.58c.205-1.909 1.954-2.476 3.719-2.942l.406-.107c.609-.158 1.205-.316 1.725-.525c7.414-3.148 16.253-4.29 27.667-4.004c-.43-2.866-7.562-6.437-9.839-8.153c-4.57-3.294-9.409-6.731-14.257-9.729c-2.569-1.57-6.996-2.716-9.842-3.999c-3.851-1.574-12.41-3.147-14.544-6.145c-3.625-4.726-6.229-10.363-8.757-16.057l-.688-1.554a803.85 803.85 0 0 0-.69-1.553c-2.988-6.857-6.7-14.006-9.695-21.027c-1.566-3.425-2.285-6.431-4-9.716c-10.407-20.158-25.81-37.035-44.485-48.904c-6.137-3.862-12.98-7.436-20.534-9.865c-4.281-1.293-9.419-.578-13.98-1.57h-3.002c-2.562-.722-4.701-3.438-6.7-4.87c-4.415-2.998-8.837-5.011-14.117-7.15c-1.85-.858-7.133-2.856-8.977-1.283c-1.142.287-1.721.718-2.002 1.864c-1.136 1.71-.137 4.286.57 5.863c2.142 4.57 5.134 7.286 7.85 11.148c2.416 3.425 5.417 7.287 7.13 11.011c3.696 8.005 5.417 16.874 8.842 24.878c1.27 3.01 3.279 6.435 5.128 9.15c1.567 2.155 4.416 3.713 5.278 6.441c1.718 2.86-2.572 12.297-3.565 15.294c-3.715 11.727-2.995 28.028 1.283 38.193l.228.536l.228.543c1.562 3.723 3.234 7.732 7.387 8.773c.286-.284 0-.135.567-.284c1.005-7.868 1.288-15.445 4-21.601c1.567-3.849 4.696-6.57 6.841-9.712c1.43.856 1.43 3.437 2.282 5.145c1.856 4.43 3.849 9.287 6.137 13.73c4.696 9.15 9.98 18.021 15.967 26.025c2.005 2.859 4.85 6.006 7.416 8.581c1.143.997 2.423 1.573 3.282 2.856h.28v.432c-4.278-1.577-6.99-6.003-10.402-8.587c-6.424-4.857-14.117-12.151-18.545-19.15c-1.852-4.018-3.854-7.869-5.85-11.867v-.289c-.853 1.142-.567 2.276-.994 4.004c-1.852 7.145-.426 15.296-6.843 17.866c-7.274 3.01-12.7-4.857-14.977-8.432c-7.276-11.866-9.269-31.884-4.138-48.043c1.14-3.577 1.295-7.867 3.285-10.723c-.43-2.582-2.42-3.288-3.571-4.87c-1.996-2.704-3.705-5.854-5.268-8.857c-3.002-5.866-5.138-12.875-7.417-19.166c-1.002-2.569-1.289-5.148-2.288-7.58c-1.704-3.712-4.845-7.436-7.268-10.72c-3.281-4.72-12.837-13.868-8.985-23.168Zm46.772 28.015c.381.382.841.716 1.317 1.045l.574.394c.765.53 1.506 1.088 1.96 1.848c.72 1.006.854 1.999 1.716 3.007c0 3.437-.996 5.722-3.007 7.146c0 0-.137.15-.278.29c-1.14-2.291-2.139-4.57-3.287-6.859c-1.414-1.998-3.413-3.583-4.565-5.866h-.277v-.287c1.721-.425 3.428-.718 5.847-.718Z" />
        </svg>
        &nbsp; Manejo de herramienta de Base de datos Mysql
      </div>

      <div class="mb-3 flex items-center">
        <img src="./assets/cSharp.png" width="50" alt="50">
        &nbsp; Master in C#
      </div>
      <div class="mb-3 flex items-center">
        <svg xmlns="http://www.w3.org/2000/svg" height="30" width="30"
          viewBox="0 0 384 512"><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
          <path
            d="M277.74 312.9c9.8-6.7 23.4-12.5 23.4-12.5s-38.7 7-77.2 10.2c-47.1 3.9-97.7 4.7-123.1 1.3-60.1-8 33-30.1 33-30.1s-36.1-2.4-80.6 19c-52.5 25.4 130 37 224.5 12.1zm-85.4-32.1c-19-42.7-83.1-80.2 0-145.8C296 53.2 242.84 0 242.84 0c21.5 84.5-75.6 110.1-110.7 162.6-23.9 35.9 11.7 74.4 60.2 118.2zm114.6-176.2c.1 0-175.2 43.8-91.5 140.2 24.7 28.4-6.5 54-6.5 54s62.7-32.4 33.9-72.9c-26.9-37.8-47.5-56.6 64.1-121.3zm-6.1 270.5a12.19 12.19 0 0 1-2 2.6c128.3-33.7 81.1-118.9 19.8-97.3a17.33 17.33 0 0 0-8.2 6.3 70.45 70.45 0 0 1 11-3c31-6.5 75.5 41.5-20.6 91.4zM348 437.4s14.5 11.9-15.9 21.2c-57.9 17.5-240.8 22.8-291.6.7-18.3-7.9 16-19 26.8-21.3 11.2-2.4 17.7-2 17.7-2-20.3-14.3-131.3 28.1-56.4 40.2C232.84 509.4 401 461.3 348 437.4zM124.44 396c-78.7 22 47.9 67.4 148.1 24.5a185.89 185.89 0 0 1-28.2-13.8c-44.7 8.5-65.4 9.1-106 4.5-33.5-3.8-13.9-15.2-13.9-15.2zm179.8 97.2c-78.7 14.8-175.8 13.1-233.3 3.6 0-.1 11.8 9.7 72.4 13.6 92.2 5.9 233.8-3.3 237.1-46.9 0 0-6.4 16.5-76.2 29.7zM260.64 353c-59.2 11.4-93.5 11.1-136.8 6.6-33.5-3.5-11.6-19.7-11.6-19.7-86.8 28.8 48.2 61.4 169.5 25.9a60.37 60.37 0 0 1-21.1-12.8z" />
        </svg>
        &nbsp; Concimientos en Java
      </div>
      <div class="mb-3 flex items-center">
        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 256 256">
          <defs>
            <linearGradient id="IconifyId18ba52d726c7a96db0" x1="50%" x2="50%" y1=".002%" y2="100%">
              <stop offset="0%" stop-color="#FFF" />
              <stop offset="100%" stop-color="#FFF" stop-opacity="0" />
            </linearGradient>
          </defs>
          <path fill="#52218A"
            d="M36.987 200.406a10.667 10.667 0 0 1-11.04 1.734L6.56 194.006A10.667 10.667 0 0 1 0 184.22V70.46a10.667 10.667 0 0 1 6.56-9.787l19.387-8a10.667 10.667 0 0 1 11.04 1.733l4.346 3.6a5.893 5.893 0 0 0-9.333 4.8v129.067a5.893 5.893 0 0 0 9.333 4.8l-4.346 3.733Z" />
          <path fill="#6C33AF"
            d="M6.56 194.006A10.667 10.667 0 0 1 0 184.22v-.88a6.16 6.16 0 0 0 10.667 4.133L176 4.673a16 16 0 0 1 18.187-3.093l52.746 25.386A16 16 0 0 1 256 41.393v.613a10.107 10.107 0 0 0-16.507-7.813l-198.16 162.48l-4.346 3.733a10.667 10.667 0 0 1-11.04 1.734L6.56 194.006Z" />
          <path fill="#854CC7"
            d="M6.56 60.673A10.667 10.667 0 0 0 0 70.46v.88a6.16 6.16 0 0 1 10.667-4.134L176 250.006a16 16 0 0 0 18.187 3.094l52.746-25.387A16 16 0 0 0 256 213.286v-.613a10.107 10.107 0 0 1-16.507 7.813L41.333 58.006l-4.346-3.733a10.667 10.667 0 0 0-11.04-1.6l-19.387 8Z" />
          <path fill="#B179F1"
            d="M194.187 253.1A16 16 0 0 1 176 250.006a9.387 9.387 0 0 0 16-6.64v-232a9.387 9.387 0 0 0-16-6.693a16 16 0 0 1 18.187-3.093l52.746 25.36A16 16 0 0 1 256 41.366v171.947a16 16 0 0 1-9.067 14.427l-52.746 25.36Z" />
          <path fill="url(#IconifyId18ba52d726c7a96db0)" fill-opacity=".25"
            d="M183.707 254.273a16 16 0 0 0 10.48-1.173l52.746-25.36A16 16 0 0 0 256 213.313V41.366a16 16 0 0 0-9.067-14.426L194.187 1.58A16 16 0 0 0 182.24.806A16 16 0 0 0 176 4.673L90.987 98.7L41.333 58.006l-4.346-3.733a10.667 10.667 0 0 0-9.627-2.213a6.8 6.8 0 0 0-1.413.48L6.56 60.673A10.667 10.667 0 0 0 0 69.66v115.36a10.664 10.664 0 0 0 6.56 8.986l19.387 8a6.8 6.8 0 0 0 1.413.48c3.378.882 6.973.056 9.627-2.213l4.346-3.6l49.654-40.693L176 250.006a16 16 0 0 0 7.707 4.267ZM192 73.153l-66.107 54.187L192 181.526V73.153ZM32 90.726l33.093 36.614L32 163.953V90.726Z" />
        </svg>
        &nbsp; Manejo de visual estudio, visual estudio code y JavaNetbeans



      </div>
      <p class="text-3xl font-bold mt-8">Habilidades Personales</p>
      <hr class="bg-green-600 h-1 w-96">
      <div class="mb-3 mt-5 flex items-center ">
        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="28" viewBox="0 0 1920 1792">
          <path fill="#000000"
            d="M593 896q-162 5-265 128H194q-82 0-138-40.5T0 865q0-353 124-353q6 0 43.5 21t97.5 42.5T384 597q67 0 133-23q-5 37-5 66q0 139 81 256zm1071 637q0 120-73 189.5t-194 69.5H523q-121 0-194-69.5T256 1533q0-53 3.5-103.5t14-109T300 1212t43-97.5t62-81t85.5-53.5T602 960q10 0 43 21.5t73 48t107 48t135 21.5t135-21.5t107-48t73-48t43-21.5q61 0 111.5 20t85.5 53.5t62 81t43 97.5t26.5 108.5t14 109t3.5 103.5zM640 256q0 106-75 181t-181 75t-181-75t-75-181t75-181T384 0t181 75t75 181zm704 384q0 159-112.5 271.5T960 1024T688.5 911.5T576 640t112.5-271.5T960 256t271.5 112.5T1344 640zm576 225q0 78-56 118.5t-138 40.5h-134q-103-123-265-128q81-117 81-256q0-29-5-66q66 23 133 23q59 0 119-21.5t97.5-42.5t43.5-21q124 0 124 353zm-128-609q0 106-75 181t-181 75t-181-75t-75-181t75-181t181-75t181 75t75 181z" />
        </svg>
        &nbsp; Trabajo en Equipo
      </div>

      <div class="mb-3 mt-5 flex items-center ">
        <svg xmlns="http://www.w3.org/2000/svg" height="1em"
          viewBox="0 0 512 512"><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
          <path
            d="M352 320c88.4 0 160-71.6 160-160c0-15.3-2.2-30.1-6.2-44.2c-3.1-10.8-16.4-13.2-24.3-5.3l-76.8 76.8c-3 3-7.1 4.7-11.3 4.7H336c-8.8 0-16-7.2-16-16V118.6c0-4.2 1.7-8.3 4.7-11.3l76.8-76.8c7.9-7.9 5.4-21.2-5.3-24.3C382.1 2.2 367.3 0 352 0C263.6 0 192 71.6 192 160c0 19.1 3.4 37.5 9.5 54.5L19.9 396.1C7.2 408.8 0 426.1 0 444.1C0 481.6 30.4 512 67.9 512c18 0 35.3-7.2 48-19.9L297.5 310.5c17 6.2 35.4 9.5 54.5 9.5zM80 408a24 24 0 1 1 0 48 24 24 0 1 1 0-48z" />
        </svg>
        &nbsp; Resolucion de problemas
      </div>

      <div class="mb-3 mt-5 flex items-center ">
        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 128 128">
          <defs>
            <path id="IconifyId18ba52d726c7a96db2"
              d="M64.41 7.92c-46.76 0-43.24 46.43-43.24 46.43s12.21-10.8 43.5-10.8s42.1 9.8 42.1 9.8s4.4-45.43-42.36-45.43z" />
          </defs>
          <path fill="#543930"
            d="M98.54 79.48c-1.28-2.3.24-4.63 2.84-7.5c4.12-4.54 8.41-14.96 3.88-25.4c.02-.06-.88-1.8-.86-1.86l-1.74-.09c-.56-.08-19.6-.12-38.67-.12s-38.11.04-38.67.12c0 0-2.62 1.89-2.59 1.95c-4.53 10.44-.24 20.86 3.88 25.4c2.61 2.87 4.12 5.2 2.84 7.5c-1.25 2.24-4.83 2.57-4.83 2.57s.77 2.3 2.73 3.54c1.82 1.15 4.07 1.42 5.64 1.46c0 0 5.98 8.75 22.05 8.75h17.9c16.08 0 22.05-8.75 22.05-8.75c1.57-.04 3.82-.31 5.64-1.46c1.96-1.24 2.73-3.54 2.73-3.54s-3.58-.33-4.82-2.57z" />
          <radialGradient id="IconifyId18ba52d726c7a96db3" cx="99.719" cy="45.167" r="23.635"
            gradientTransform="matrix(1 0 0 .4912 -21.218 59.61)" gradientUnits="userSpaceOnUse">
            <stop offset=".728" stop-color="#6D4C41" stop-opacity="0" />
            <stop offset="1" stop-color="#6D4C41" />
          </radialGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db3)"
            d="M63.99 95.79v-9.7l28.35-2.31l2.64 3.26S89 95.79 72.93 95.79h-8.94z" />
          <radialGradient id="IconifyId18ba52d726c7a96db4" cx="78.394" cy="52.487" r="6.935"
            gradientTransform="matrix(-.9057 .4238 -.3144 -.6719 188.815 80.345)" gradientUnits="userSpaceOnUse">
            <stop offset=".663" stop-color="#6D4C41" />
            <stop offset="1" stop-color="#6D4C41" stop-opacity="0" />
          </radialGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db4)"
            d="M94.84 82.84c-4.36-6.58 4.96-9.02 4.96-9.02l.01.01c-1.58 2.07-2.28 3.87-1.29 5.66c1.25 2.24 4.83 2.57 4.83 2.57s-4.76 4.43-8.51.78z" />
          <radialGradient id="IconifyId18ba52d726c7a96db5" cx="97.159" cy="68.993" r="30.046"
            gradientTransform="matrix(-.0746 -.9972 .8311 -.0622 33.045 160.08)" gradientUnits="userSpaceOnUse">
            <stop offset=".725" stop-color="#6D4C41" stop-opacity="0" />
            <stop offset="1" stop-color="#6D4C41" />
          </radialGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db5)"
            d="M105.28 46.58c4.42 10.11.34 20.66-3.66 25.15c-.55.62-2.9 3.02-3.4 4.95c0 0-9.64-13.31-12.54-21.02c-.58-1.55-1.11-3.14-1.2-4.78c-.07-1.23.1-2.68.79-3.72c.84-1.27 19.49-1.65 19.49-1.65c0 .01.52 1.07.52 1.07z" />
          <radialGradient id="IconifyId18ba52d726c7a96db6" cx="46.811" cy="71.386" r="30.046"
            gradientTransform="matrix(.0746 -.9972 -.8311 -.0622 100.723 110.021)" gradientUnits="userSpaceOnUse">
            <stop offset=".725" stop-color="#6D4C41" stop-opacity="0" />
            <stop offset="1" stop-color="#6D4C41" />
          </radialGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db6)"
            d="M22.74 46.58c-4.42 10.11-.34 20.66 3.66 25.15c.55.62 2.9 3.02 3.4 4.95c0 0 9.64-13.31 12.54-21.02c.58-1.55 1.11-3.14 1.2-4.78c.07-1.23-.1-2.68-.79-3.72c-.84-1.27-1.85-.93-3.32-.93c-2.79 0-14.98-.71-15.93-.71c.01 0-.76 1.06-.76 1.06z" />
          <radialGradient id="IconifyId18ba52d726c7a96db7" cx="51.52" cy="45.167" r="23.635"
            gradientTransform="matrix(-1 0 0 .4912 101.041 59.61)" gradientUnits="userSpaceOnUse">
            <stop offset=".728" stop-color="#6D4C41" stop-opacity="0" />
            <stop offset="1" stop-color="#6D4C41" />
          </radialGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db7)"
            d="M64.03 95.79v-9.7l-28.35-2.31l-2.64 3.26s5.98 8.75 22.05 8.75h8.94z" />
          <radialGradient id="IconifyId18ba52d726c7a96db8" cx="26.384" cy="51.344" r="6.934"
            gradientTransform="matrix(.9057 .4238 .3144 -.6719 -13.326 101.62)" gradientUnits="userSpaceOnUse">
            <stop offset=".663" stop-color="#6D4C41" />
            <stop offset="1" stop-color="#6D4C41" stop-opacity="0" />
          </radialGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db8)"
            d="M33.18 82.84c4.36-6.58-4.96-9.02-4.96-9.02l-.01.01c1.58 2.07 2.28 3.87 1.29 5.66c-1.25 2.24-4.83 2.57-4.83 2.57s4.76 4.43 8.51.78z" />
          <linearGradient id="IconifyId18ba52d726c7a96db9" x1="64" x2="64" y1="34.219" y2="-7.598"
            gradientTransform="matrix(1 0 0 -1 0 130)" gradientUnits="userSpaceOnUse">
            <stop offset=".003" stop-color="#448AFF" />
            <stop offset=".404" stop-color="#3B7DFF" />
            <stop offset="1" stop-color="#2962FF" />
          </linearGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db9)"
            d="M116 123.96v-3.12c0-16.78-26.35-25.09-52-25.06c-25.49.03-52 7.28-52 25.06v3.12h104z" />
          <path fill="#99674F"
            d="M64 90.11h-9.08v9.98c0 4.51 3.7 8.17 8.26 8.17h1.65c4.56 0 8.26-3.66 8.26-8.17v-9.98H64z" />
          <linearGradient id="IconifyId18ba52d726c7a96db10" x1="44.025" x2="44.025" y1="31.848" y2="-5.871"
            gradientTransform="matrix(1 0 0 -1 0 130)" gradientUnits="userSpaceOnUse">
            <stop offset=".003" stop-color="#FF9100" />
            <stop offset=".44" stop-color="#FF8400" />
            <stop offset="1" stop-color="#FF6D00" />
          </linearGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db10)"
            d="M58.5 123.96H29.55v-22.74c1.79-.7 3.68-1.32 5.64-1.86c2.89-.82 5.94-1.49 9.1-2.01c2.14-.36 4.33-.65 6.55-.88l7.66 27.49z" />
          <linearGradient id="IconifyId18ba52d726c7a96db11" x1="43.075" x2="43.075" y1="32.056" y2="-6.655"
            gradientTransform="matrix(1 0 0 -1 0 130)" gradientUnits="userSpaceOnUse">
            <stop offset=".003" stop-color="#FF0" />
            <stop offset=".678" stop-color="#FFF200" />
            <stop offset="1" stop-color="#FFEA00" />
          </linearGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db11)"
            d="M50.96 123.96H36.29l-1.1-24.6c2.89-.82 5.94-1.49 9.1-2.01l6.67 26.61z" />
          <linearGradient id="IconifyId18ba52d726c7a96db12" x1="83.975" x2="83.975" y1="31.811" y2="-5.853"
            gradientTransform="matrix(1 0 0 -1 0 130)" gradientUnits="userSpaceOnUse">
            <stop offset=".003" stop-color="#FF9100" />
            <stop offset=".44" stop-color="#FF8400" />
            <stop offset="1" stop-color="#FF6D00" />
          </linearGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db12)"
            d="M98.45 101.52v22.44H69.5l7.65-27.45c2.21.24 4.4.56 6.53.94c3.17.57 6.23 1.27 9.12 2.13c1.97.58 3.86 1.23 5.65 1.94z" />
          <linearGradient id="IconifyId18ba52d726c7a96db13" x1="84.92" x2="84.92" y1="31.958" y2="-6.607"
            gradientTransform="matrix(1 0 0 -1 0 130)" gradientUnits="userSpaceOnUse">
            <stop offset=".003" stop-color="#FF0" />
            <stop offset=".678" stop-color="#FFF200" />
            <stop offset="1" stop-color="#FFEA00" />
          </linearGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db13)"
            d="m92.8 99.58l-1.09 24.38H77.04l6.64-26.51c3.17.57 6.23 1.27 9.12 2.13z" />
          <path fill="#99674F"
            d="M91.33 50.43H36.67c-5.89 0-10.71 5.14-10.71 11.41s4.82 11.41 10.71 11.41h54.65c5.89 0 10.71-5.14 10.71-11.41s-4.81-11.41-10.7-11.41z" />
          <path fill="#BA8D68"
            d="M64 11.11c-17.4 0-33.52 18.61-33.52 45.39c0 26.64 16.61 39.81 33.52 39.81S97.52 83.14 97.52 56.5c0-26.78-16.12-45.39-33.52-45.39z" />
          <g fill="#49362E">
            <ellipse cx="48.4" cy="58.81" rx="4.93" ry="5.1" />
            <ellipse cx="79.6" cy="58.81" rx="4.93" ry="5.1" />
          </g>
          <path fill="#613E31"
            d="M55.64 49.82c-.93-1.23-3.07-3.01-7.23-3.01s-6.31 1.79-7.23 3.01c-.41.54-.31 1.17-.02 1.55c.26.35 1.04.68 1.9.39s2.54-1.16 5.35-1.18c2.81.02 4.49.89 5.35 1.18c.86.29 1.64-.03 1.9-.39c.28-.38.39-1.01-.02-1.55zm31.19 0c-.93-1.23-3.07-3.01-7.23-3.01s-6.31 1.79-7.23 3.01c-.41.54-.31 1.17-.02 1.55c.26.35 1.04.68 1.9.39s2.54-1.16 5.35-1.18c2.81.02 4.49.89 5.35 1.18c.86.29 1.64-.03 1.9-.39c.28-.38.39-1.01-.02-1.55z" />
          <path fill="#99674F"
            d="M67.86 68.06c-.11-.04-.21-.07-.32-.08h-7.08c-.11.01-.22.04-.32.08c-.64.26-.99.92-.69 1.63c.3.71 1.71 2.69 4.55 2.69s4.25-1.99 4.55-2.69c.3-.71-.05-1.37-.69-1.63z" />
          <path fill="#5D4037"
            d="M72.42 76.14c-3.18 1.89-13.63 1.89-16.81 0c-1.83-1.09-3.7.58-2.94 2.24c.75 1.63 6.44 5.42 11.37 5.42s10.55-3.79 11.3-5.42c.75-1.66-1.09-3.33-2.92-2.24z" />
          <path fill="#543930"
            d="M64 11.07c-37.26 0-34.74 41.41-34.74 41.41c-.46 8.01 1.03 12.94 1.82 14.93c.14.35.63.32.72-.04c.99-3.97 4.36-17.8 4.03-20.21c0 0 11.35-2.25 22.17-10.22c2.2-1.62 4.59-3 7.13-4.01c13.59-5.41 16.43 3.82 16.43 3.82s9.42-1.81 12.26 11.27c1.07 4.9 1.79 12.75 2.4 18.24c.04.39.57.47.72.11c.95-2.18 3.1-6.48 3.3-10.91c.69-14.78-8.06-44.39-36.24-44.39z" />
          <radialGradient id="IconifyId18ba52d726c7a96db14" cx="64" cy="98.344" r="34.638"
            gradientTransform="matrix(1 0 0 -.9806 0 127.575)" gradientUnits="userSpaceOnUse">
            <stop offset="0" stop-color="#FF0" />
            <stop offset=".325" stop-color="#FFF600" />
            <stop offset=".854" stop-color="#FFDE00" />
            <stop offset=".998" stop-color="#FFD600" />
          </radialGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db14)"
            d="M64.41 7.92c-46.76 0-43.24 46.43-43.24 46.43s12.21-10.8 43.5-10.8s42.1 9.8 42.1 9.8s4.4-45.43-42.36-45.43z" />
          <linearGradient id="IconifyId18ba52d726c7a96db15" x1="64.322" x2="64.322" y1="125.265" y2="88.227"
            gradientTransform="matrix(1 0 0 -1 0 130)" gradientUnits="userSpaceOnUse">
            <stop offset="0" stop-color="#FF0" />
            <stop offset=".446" stop-color="#FFF900" />
            <stop offset="1" stop-color="#FFEA00" />
          </linearGradient>
          <path fill="url(#IconifyId18ba52d726c7a96db15)"
            d="M74.72 42.15h-20.8V9.05c0-2.79 2.26-5.05 5.05-5.05h10.7c2.79 0 5.05 2.26 5.05 5.05v33.1z" />
          <clipPath id="IconifyId18ba52d726c7a96db16">
            <use href="#IconifyId18ba52d726c7a96db2" />
          </clipPath>
          <path fill="#EDBF00" d="m47.12 10.23l6.95-2.3v23.29c-1.41-10.07-3.43-17.35-6.95-20.99z"
            clip-path="url(#IconifyId18ba52d726c7a96db16)" opacity=".5" />
          <defs>
            <use href="#IconifyId18ba52d726c7a96db2" id="IconifyId18ba52d726c7a96db17" />
          </defs>
          <clipPath id="IconifyId18ba52d726c7a96db18">
            <use href="#IconifyId18ba52d726c7a96db17" />
          </clipPath>
          <path fill="#EDBF00" d="m81.71 10.23l-6.95-2.3v23.29c1.41-10.07 3.43-17.35 6.95-20.99z"
            clip-path="url(#IconifyId18ba52d726c7a96db18)" opacity=".5" />
          <path fill="#FFEA00"
            d="M109.53 50.99c-.25-.42-.64-.74-1.07-.99C90.36 39.58 64 39.93 64 39.93S37.64 39.58 19.54 50c-.44.25-.82.57-1.07.99c-.53.9-.97 2.61.75 5.08c0 0 10.95-10.85 44.78-11.53c33.82.69 44.78 11.53 44.78 11.53c1.72-2.47 1.28-4.18.75-5.08z" />
        </svg>
        &nbsp; Proactivo
      </div>

      <div class="mb-3 mt-5 flex items-center ">
        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 16 16">
          <g fill="#000000">
            <path
              d="M8.069 0c.262 0 .52.017.76.057a4.1 4.1 0 0 1 .697.154c.228.069.451.155.674.263c.217.103.44.229.663.366c.377.24.748.434 1.126.589a7.537 7.537 0 0 0 2.331.525c.406.029.823.046 1.257.046v4c0 .76-.097 1.48-.291 2.166a8.996 8.996 0 0 1-.789 1.943a10.312 10.312 0 0 1-1.188 1.725a15.091 15.091 0 0 1-1.492 1.532a17.57 17.57 0 0 1-1.703 1.325c-.594.412-1.194.795-1.794 1.143l-.24.143l-.24-.143a27.093 27.093 0 0 1-1.806-1.143a15.58 15.58 0 0 1-1.703-1.325a15.082 15.082 0 0 1-1.491-1.532a10.947 10.947 0 0 1-1.194-1.725a9.753 9.753 0 0 1-.789-1.943A7.897 7.897 0 0 1 .571 6V2c.435 0 .852-.017 1.258-.046a8.16 8.16 0 0 0 1.188-.171c.383-.086.766-.2 1.143-.354A6.563 6.563 0 0 0 5.28.846C5.72.56 6.166.349 6.606.21A4.79 4.79 0 0 1 8.069 0zm6.502 2.983a9.566 9.566 0 0 1-2.234-.377a7.96 7.96 0 0 1-2.046-.943A4.263 4.263 0 0 0 9.23 1.16A3.885 3.885 0 0 0 8.074.994a3.99 3.99 0 0 0-1.165.166a3.946 3.946 0 0 0-1.058.503A7.926 7.926 0 0 1 3.8 2.61c-.709.206-1.451.332-2.229.378v3.017c0 .663.086 1.297.258 1.908a8.58 8.58 0 0 0 .72 1.743a9.604 9.604 0 0 0 1.08 1.572c.417.491.862.948 1.342 1.382c.48.435.983.835 1.509 1.206c.531.372 1.063.709 1.594 1.017a22.397 22.397 0 0 0 1.589-1.017a15.389 15.389 0 0 0 1.514-1.206c.48-.434.926-.891 1.343-1.382a9.596 9.596 0 0 0 1.08-1.572a8.258 8.258 0 0 0 .709-1.743a6.814 6.814 0 0 0 .262-1.908V2.983z" />
            <path fill-rule="evenodd"
              d="m11.797 4.709l-.44-.378l-.406.035l-4.36 5.148l-1.485-2.12l-.4-.068l-.463.331l-.069.4l1.909 2.726l.217.12l.457.028l.234-.102l4.835-5.715l-.029-.405z"
              clip-rule="evenodd" />
          </g>
        </svg>
        &nbsp; Responsable
      </div>

      <div class="mb-3 mt-5 flex items-center ">
        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24">
          <path fill="#000000"
            d="M6.707 3.293a1 1 0 0 0-1.414 0L4 4.586l-.293-.293a1 1 0 0 0-1.414 1.414l1 1a1 1 0 0 0 1.414 0l2-2a1 1 0 0 0 0-1.414Zm14.296 13.7H10L9.883 17A1 1 0 0 0 10 18.993h11.003l.117-.006a1 1 0 0 0-.117-1.994Zm0-5.993H10l-.117.007A1 1 0 0 0 10 13h11.003l.117-.007A1 1 0 0 0 21.003 11Zm0-6H10l-.117.007A1 1 0 0 0 10 7h11.003l.117-.007A1 1 0 0 0 21.003 5ZM6.707 16.293a1 1 0 0 0-1.414 0L4 17.586l-.293-.293a1 1 0 0 0-1.414 1.414l1 1a1 1 0 0 0 1.414 0l2-2a1 1 0 0 0 0-1.414Zm-1.414-6.5a1 1 0 0 1 1.414 1.414l-2 2a1 1 0 0 1-1.414 0l-1-1a1 1 0 1 1 1.414-1.414l.293.293l1.293-1.293Z" />
        </svg> &nbsp; Organizado
      </div>

      <div class="mb-3 mt-5 flex items-center ">
        <img class="w-10 object-cover rounded-xl" src="./assets/idioma.png" alt="30">
         &nbsp; Languagues English level: Advanced, Russian level: Beginner
      </div>


    </div>



  </div>
  <div class="flex justify-center items-center">
    <div class="flex-1 max-w-7x1 p-5">
      <!-- <hr class="bg-blue-600 h-1 w-96"> -->
      <h3 class="flex justify-center text-2xl mb-5 ml-20 font-bold">Personal Projects</h3>
      
      
      <p class="flex justify-center">here's a short list of ones of my personal projects</p>
      <br>
      <div class="grid grid-cols-3 grid-rows-1 gap-5 grid-flow-row-dense">
        
        <div class="p-4 pr-6 bg-gray-200 border-4 border-green-500 rounded-md shadow-md space-y-2">

        
          <h1 class="font-bold">Environmet Project</h1>
          <img class="w-80 h-15 object-cover rounded-xl ml-5" src="./assets/medoAmbiente.jpg">
          <p>this a project created in order to help who wants to protect the environmet through connecting people from
            differente places with the objective of restoring the most damaged environments in their areas</p>
        </div>
        <div class="p-4 pr-6 bg-gray-200 border-4 border-green-500 rounded-md shadow-md space-y-2">


          <br>
          <h1 class="font-bold">Restaurant App</h1>
          <img class="w-40 h-15 object-cover rounded-xl ml-20" src="./assets/restaurante.jpg">
          <p>This is an App mobile created to help the employees of a restaurant to have a better control of the
            reservations the have
            and also to give the custumers the option of doing they themselves their reservations using the app
          </p>
        </div>
        <div class="p-4 pr-6 bg-gray-200 border-4 border-green-500 rounded-md shadow-md space-y-2">

          <br>
          <h1 class="font-bold">Library Project</h1>
          <img class="w-40 h-15 object-cover rounded-xl ml-20" src="./assets/bookstore.jpg">
          <p>This is project created in order to help the Library in the control of the books that have been leant
            to prevent errror and have the control of how many books in specific they have and
          </p>
        </div>

      </div>
    </div>
  </div>
</div>
</template>



  